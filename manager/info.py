...
@bp.route('/multi_delete', methods=('GET', 'POST'))
def multi_delete():
    if request.method == 'POST':
        form_dict = request.form.to_dict()
    ...
    if request.method == 'POST':
        ...
        for filename in old_filename_list:
            if (filename not in contract_filename_list) or (filename in contract_file_list):
                delete_path = os.path.join(app.config['UPLOAD_FOLDER'], str(id), filename)
        ...
        for photoname in old_photoname_list:
            if (photoname not in photoname_list) or (photoname in photo_list):
                delete_path = os.path.join(app.config['UPLOAD_FOLDER'], str(id), photoname)
                if os.path.exists(delete_path):
                    os.remove(delete_path)
        index = 0
        ...
        cursor = db.cursor()
        cursor.execute(
            "UPDATE `information` SET `type`=%s, `name`=%s, `model`=%s, `serial_number`=%s, `contract_number`=%s, `arrival_date`=%s, `transactor`=%s, `status`=%s, `location`=%s, `contract_files`=%s, `photo`=%s, `location_photo`=%s, `comment`=%s WHERE `id`=%s",
            [new_record['type'], new_record['name'], new_record['model'], new_record['serial_number'], new_record['contract_number'], new_record['arrival_date'], new_record['transactor'], new_record['status'], new_record['location'], new_record['contract_files'], new_record['photo'], new_record['location_photo'], new_record['comment'], int(id)]
        )
        cursor.close()
        db.commit()
    record = get_record(id)
...
def import_excel():
    if request.method == 'POST':
        ...
        if xlsfile.filename != '':
            ...
            cursor.close()
            return redirect(url_for('index'))
    return render_template('info/import_excel.html')